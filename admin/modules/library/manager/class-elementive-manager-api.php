...
namespace Elementive\Admin\Modules\Library\Manager;

defined( 'ABSPATH' ) || exit;
class Elementive_Manager_API {
	...
	public function __construct() {
		...
		$this->sources = array( 'elementive-api' );
		$this->server_url = 'https://api.wpmet.com/public/elementor-layouts/';
		$this->dir = dirname( __FILE__ ) . '/';
		$this->url = ELEMENTIVE_PATH . 'modules/library/templates/';

		add_action( 'wp_ajax_elementive_get_templates', array( $this, 'get_templates' ) );
		add_action( 'wp_ajax_elementive_core_clone_template', array( $this, 'clone_template' ) );
		add_action( 'wp_ajax_elementive_get_layouts', array( $this, 'get_layouts' ) );
	...
	public function get_license() {
		if ( ! class_exists( '\Elementive\Libs\Framework\Classes\tils' ) ) {
			return array();
		}

		return array(
			'key' => \Elementive\Libs\Framework\Classes\Utils::instance()->get_option( 'license_key' ),
	...
	public function get_layouts() {
		...
		$query = array_merge(
			array(
				'action' => 'get_layouts',
				'tab' => ( empty( $_GET['tab'] ) ? 'elementive_page' : $_GET['tab'] ),
			),
			$this->get_license()
		);
		...
		if ( $response['body'] != '' ) {
			echo \Elementive\Utils::kses( $response['body'] );
			exit;
		}
	}
