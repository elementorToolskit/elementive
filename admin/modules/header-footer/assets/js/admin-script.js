jQuery(document).ready(function ($) {
    ...
    // $('.elementive-select2').select2({
    //     dropdownParent: $('#elementive_headerfooter_modal'),
    //     placeholder: "--",
    // });
    ...
    $('.row-actions .edit a, .page-title-action, .column-title .row-title').on('click', function (e) {
        e.preventDefault();
        var id = 0;
        var modal = $('#elementive_headerfooter_modal');
        ...
        if (parent.length > 0) {
            ...
            $.get(window.elementive.resturl + 'my-template/get/' + id, function (data) {
                Elementive_Template_Editor(data);
    ...
    $('.elementive-template-save-btn-editor').on('click', function () {
        var form = $('#elementive-template-modalinput-form');
        form.attr('data-open-editor', '1');
        form.trigger('submit');
    });

    $('#elementive-template-modalinput-form').on('submit', function (e) {
        e.preventDefault();
        var modal = $('#elementive_headerfooter_modal');
        modal.addClass('loading');
        ...
        $.get(window.elementive.resturl + 'my-template/update/' + id, form_data, function (output) {
            //console.log(output);
            modal.removeClass('loading');
    ...
    $('.ekit-template-modalinput-condition_singular_id').select2({
        ajax: {
            url: window.elementive.resturl + 'ajaxselect2/singular_list',
            dataType: 'json',
        ...
        cache: true,
        placeholder: "--",
        dropdownParent: $('#elementive_headerfooter_modal_body')
        //minimumInputLength: 2,
    });

    function Elementive_Template_Editor(data) {
	...
	var s = url.searchParams.get("elementive_type_filter");
	s = (s == null) ? 'all' : s;

	$.each(xs_types, function(k, v){
		var url = ekit_url_replace_param(window.location.href, 'elementive_type_filter', k);
        var klass = (s == k) ? 'elementive_type_filter_active nav-tab-active' : ' ';
        tabs += `
            <a href="${url}" class="${klass} elementive_type_filter_tab_item nav-tab">${v}</a>
        `;
        tabs += "\n";
	});
	tab_container.after('<div class="elementive_type_filter_tab_container nav-tab-wrapper">'+ tabs +'</div><br/>');
});